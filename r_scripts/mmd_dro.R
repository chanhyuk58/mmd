...
mmd_dro <- function(formula, data, v0_col, v1_col,
  ...
  if(verbose) cat(">> [1/4] Preparing and validating data...\n")
  ...
  # SD is calculated over the full v0 sample
  # v0 is the lower value of the v
  ...
  solve_dro_bounds <- function(v0_s, v1_s, x_s, y_val, d_val) {
    ...
    if (method == "projection") {
      ...
      # --- PROFILE METHOD ---
      # Calculate raw point estimates for centering the grid
    ...
    # Calculate point estimate in raw scale
    theta_hat_raw <- numeric(p)
    theta_hat_raw[2] <- theta_hat_std[2] / sd_v
    if(d > 0) theta_hat_raw[3:p] <- theta_hat_std[3:p] / sd_x
    theta_hat_raw[1] <- theta_hat_std[1] - (theta_hat_raw[2] * mu_v) - sum(theta_hat_raw[3:p] * mu_x)
  ...
  # Based on Kallus and Zhou (2018) and Kaido, Molinari, and Stoye (2019)
  if (B > 0) {
...
print.summary.mmd_results <- function(x, ...) {
  ...
  cat("=================================================================\n")
  cat("Family: ", x$family, " | Scale: ", x$scale, " | n: ", x$n, "\n\n")
...
plot.mmd_results <- function(x, parameter, ...) {
  ...
  # Point estimates
  lines(sum_tab$Delta, sum_tab$Point_Est, col = "red", lty = 2, lwd = 1.5)
  
  # Dashed line at 0 (The significance threshold)
  abline(h = 0, col = "grey50", lty = 3, lwd = 1.5)
  
  # Legend
  legend("topright", legend = c("Identified Set", "Point Estimate", "Significance Limit (0)"),
         fill = c(rgb(0.2, 0.4, 0.8, 0.2), NA, NA), border = c(NA, NA, NA),
         col = c(NA, "red", "grey50"), lty = c(NA, 2, 3), lwd = c(NA, 1.5, 1.5),
         bty = "n", cex = 0.9)
}
